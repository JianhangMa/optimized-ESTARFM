...
;  Similar pixels were identified with threshold based method
;PROCEDURE:
...
;-------------------------------------------------------------------
pro  ESTARFM_ByThreshold,w,num_class,DN_min,DN_max,background,$
  FineFP1,CoarseFP1,FineFP2,CoarseFP2,CoarseFP_pre,FineFP_pre,logstr=logstr
  compile_opt idl2
  ;------------------------------------------------------------------------
  ...
  similarPixelCnts=uintarr(ns,nl)
  ...
  val=where(valid_index)
  ;compute the threshold of similar pixel seeking
  similar_th=fltarr(1,1,nb*2)
  for iband=0,nb-1,1 do begin
    similar_th[0,0,iband]   =stddev((fine1[*,*,iband])[val])*2.0/num_class   ;pair 1
    similar_th[0,0,nb+iband]=stddev((fine2[*,*,iband])[val])*2.0/num_class   ;pair 2
  endfor
  ...
  for j=0,nl-1,1 do begin        ;retieve each target pixel
    for i=0,ns-1,1 do begin
      if (valid_index[i,j] eq 1) then begin    ;do not process the background
        ...
        ;similar pixel
        ;===================================================================
        slice=rebin(similar_th,winsamples,winlines,nb*2,/SAMPLE)
        wind_fine=fltarr(winsamples,winlines,nb*2)
        wind_fine[*,*, 0:(nb-1)]  =fine1[ai:bi,aj:bj,*]
        wind_fine[*,*,nb:(2*nb-1)]=fine2[ai:bi,aj:bj,*]
        S_S=abs(wind_fine-rebin(wind_fine[ci,cj,*],winsamples,winlines,nb*2,/SAMPLE)) lt slice
        position_cand=(total(S_S,3) eq nb*2)*valid_index[ai:bi,aj:bj]
        ; subscript of similar and valid pixels in the window
        indcand=where(position_cand eq 1,number_cand)
        ...
        if (number_cand gt 5) then begin
          ...
          var1=mean(fine_cand*coarse_cand,dimension=2)-mean(fine_cand,dimension=2)*mean(coarse_cand,dimension=2)
          var2=sqrt(mean(fine_cand*fine_cand,dimension=2)-mean(fine_cand,dimension=2)^2)
          var3=sqrt(mean(coarse_cand*coarse_cand,dimension=2)-mean(coarse_cand,dimension=2)^2)
          S_D_cand=var1/(var2*var3)
          valid_S=S_D_cand ne S_D_cand
          if total(valid_S) ne 0 then S_D_cand[where(valid_S)]=0.0

          ;spatial distance
          iw=indcand mod winsamples
          jw=indcand/winsamples
          ...
          for iband=0,nb-1,1 do begin
            ;compute V
            coarse_candTemp=[coarse_cand[*,iband],coarse_cand[*,iband+nb]]    ;independent variable
            fine_candTemp  =[fine_cand[*,iband],fine_cand[*,iband+nb]]        ;dependent variable
        ...
        endif else begin   ;for the case of no similar pixel selected
          for iband=0,nb-1,1 do begin
            ...
            T_weight1=(1.0/difc_pair1)/(1.0/difc_pair1+1.0/difc_pair2)
            T_weight2=(1.0/difc_pair2)/(1.0/difc_pair1+1.0/difc_pair2)
            fine0[i,j,iband]=T_weight1*fine1[i,j,iband]+T_weight2*fine2[i,j,iband]
          endfor
        endelse
      endif
    endfor
